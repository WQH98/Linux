...
unsigned short lcd_read_panelid(void) {
    ...
    idx = (unsigned char)gpio_read_pin(GPIO3, 28); 	    /* 读取M0 */
	idx |= (unsigned char)gpio_read_pin(GPIO3, 20) << 1;	/* 读取M1 */
	idx |= (unsigned char)gpio_read_pin(GPIO3, 12) << 2;	/* 读取M2 */
