...
unsigned char icm20608_init(void) {
    ...
    /* 3、ICM20608初始化 */
    reg_value = icm20608_read_reg(ICM20_WHO_AM_I);
    if((reg_value != ICM20608D_ID) && (reg_value != ICM20608G_ID)) {
        return 1;
    }
...
unsigned char icm20608_read_reg(unsigned char reg) {
    ...
    spich0_read_write_byte(ECSPI3, reg);        // 发送要读取的寄存器地址
    reg_value = spich0_read_write_byte(ECSPI3, 0xFF);       // 从机返回寄存器数据
...
void icm20608_write_reg(unsigned char reg, unsigned char value) {
    ...
    spich0_read_write_byte(ECSPI3, reg);        // 发送要写入的寄存器地址
    spich0_read_write_byte(ECSPI3, value);      // 发送要写入的数据
...
void icm20608_read_len(unsigned char reg, unsigned char *buf, unsigned char len) {
    ...
    for(i = 0; i < len; i++) {
        buf[i] = spich0_read_write_byte(ECSPI3, 0xFF);          // 从机返回寄存器数值
    }

    ICM20608_CSN(1);                // 拉高片选 停止访问
}
